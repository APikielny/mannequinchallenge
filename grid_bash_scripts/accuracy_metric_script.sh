...
############
#IMPORTANT: when using this script, make sure the proper arguments are passed to accuracy_test_videos.py (scale, anti_alias up and downsampling)
...
model_scale=${3:-""}
manequin_repo=${4:-"/data/jhtlab/apikieln/fourier-features-mannequin"}
...
for d in */ ; do
    ...
    if [ ! -d ${tst_folder} ]; then
        cd ${manequin_repo}
        python accuracy_test_videos.py --viz_folder "accuracy_testing" --accuracy_test_list ${txt_file} --weights ${model_name} --accuracy_id ${id} --accuracy_dataset ${dataset} --scale ${model_scale} --anti_alias_downsample #>/dev/null 2>&1
...
cd ${manequin_repo}/accuracy_scripts
python accuracy_metric.py --model_name ${model_name} --dataset ${dataset}
